...
export const AlgoProvider =({children}) => {
    ...
    const optinRequest=(name,address)=>{
     setOptinList(optinList=>[...optinList, {
      id:1,
      name:name,
      address:address
     }]);
        ...
        const createAsset= async(req) => {
          ...
          try {
           ...
           const  sk = algosdk.mnemonicToSecretKey(process.env.NEXT_PUBLIC_MNEMO1);
            ...
            const upfsUrl = 'https://gateway.pinata.cloud/ipfs/QmaNsbBW2ZVwdvonkwUvYmfBvkX4HZKpgYgTV4iYMsVryw';
...
useEffect(()=> {
 ...
 ConnectAlgoSigner();
      ...
      return (
        <AlgoContext.Provider value={{CheckAlgoSigner, ConnectAlgoSigner, createAsset, currentAccount,optinList,optinRequest,setOptinList,transferAsset}}>
            {children}
